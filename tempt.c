...
// Allocate a zeroed disk block.
static uint
b3alloc(uint dev, uint prevaddr)
{
  int b, bi, m;
  struct buf *bp;
  ...
  for(b = 0; b < sb.size; b += BPB){
    ...
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
      m = 1 << (bi % 8);
      if((bp->data[bi/8] & m) == 0){  // Is block free?
		if(prevaddr == -1 || prevaddr + 1 == b+bi){ // Is block is sequential?
		  bp->data[bi/8] |= m;  // Mark block in use.
		  log_write(bp);
		  brelse(bp);
		  bzero(dev, b + bi);
...
static uint
b2alloc(uint dev, uint length)
{
  int b, bi, m;
  struct buf *bp;
  uint b_start=-1, b_length=0;
  ...
  for(b = 0; b < sb.size; b += BPB){
    ...
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
	  ...
	  else if(b_start != -1){ // not free
		end = bi;
		break;
	  }
    }
	for(ti = bst; (ti < BPB && b + ti < sb.size) && (end == -1 || ti < bi); ti++){
	  m = 1 << (ti % 8);	
	  bp->data[ti/8] |= m;  // Mark block in use.
	}
	log_write(bp);
	brelse(bp);
	for(ti = bst; (ti < BPB && b + ti < sb.size) && (end == -1 || ti < bi); ti++){
...
// [20172644] bcsmap
static uint
bcsmap(struct inode *ip, uint off)
{
  uint staddr;
  uint prevaddr;
  uint alloclen;
  ...
  if(bn != 0){ // 마지막 addrs 확장
	...
	while(alloclen < 255 && accum + alloclen <= position){  // 연장 할당
	  prevaddr = b3alloc(ip->dev, prevaddr);
	  if(prevaddr == -1)
		break;
	  ++alloclen;
	}
	if((ip->addrs[bn-1] & 255) != alloclen){ // 확장하였으면 아이노드에 저장
	  ip->addrs[bn-1] = (staddr << 8) | (alloclen & 255);
	}
	if(accum + alloclen > position){ // 데이터를 위치시킬 공간을 얻었다면
	  cprintf("realloc! %d, %d\n", staddr, alloclen);
	  return staddr + alloclen - 1;
	}
  }
  ...
  while(1){
	staddr = prevaddr = b3alloc(ip->dev, -1); // 새로 할당
	alloclen = 1;
	while(alloclen < 255 && accum + alloclen <= position){ // 연장 할당
	  prevaddr = b3alloc(ip->dev, prevaddr);
	  if(prevaddr == -1)
